...
describe("Codex model discovery request",()=>{test("sends the current Codex client version using the backend contract",()=>{const url=new URL(CODEX_MODELS_URL);const headers=codexHeaders("token","account");expect(CODEX_COMPATIBILITY.clientVersion).toBe("0.153.4");expect(url.searchParams.get("client_version")).toBe(CODEX_COMPATIBILITY.clientVersion);expect(headers.version).toBe(CODEX_COMPATIBILITY.clientVersion);expect(headers.originator).toBe("redwake");expect(headers["x-openai-originator"]).toBeUndefined();});});
...
describe("Codex Responses contract",()=>{
 ...
 test("builds default instructions with the current working directory",async()=>{let request:CodexRequest|undefined;const transport={createResponse:async(body:CodexRequest)=>{request=body;return{id:"response",status:"completed",output:[],outputText:"",reasoningSummaries:[]} satisfies CodexTurnResponse;}} as unknown as CodexTransport;const agent=new CodexAgent({transport,model:"gpt-codex-test"});await agent.createResponse();expect(request?.instructions).toContain(`Current working directory: ${process.cwd()}`);});
