...
export interface CodexAgentOptions extends AgentBaseOptions {
  transport: CodexTransport;
  model?: string;
}

export class CodexAgent extends AgentBase<CodexTurnResponse, CodexFunctionOutput> {
  ...
  constructor(private readonly options: CodexAgentOptions) {
    super(options);
    this.model = options.model ?? modelFor("openai-codex");
    this.input = toOpenAIHistory(this.conversation.snapshot(1_200_000));
  }

  private responseRequest(system?: string): CodexRequest {
    const instructions = system?.trim() ? system : this.systemPrompt?.text ?? buildSystemPrompt({ cwd: this.workspaceRoot });
    return {
      model: this.model,
      instructions,
      input: this.input as unknown[],
  ...
  protected remember(response: CodexTurnResponse): void {
    this.input.push(...response.output as ResponseInput);
  }

  protected responseText(response: CodexTurnResponse): string {
    return response.outputText;
  }

  protected *toolCalls(response: CodexTurnResponse): Iterable<NormalizedToolCall> {
    for (const item of response.output) {
      if (item.type !== "function_call" || typeof item.call_id !== "string" ||
          typeof item.name !== "string" || typeof item.arguments !== "string") continue;
      try {
  ...
  protected encodeToolResult(call: NormalizedToolCall, content: string): CodexFunctionOutput {
    return { type: "function_call_output", call_id: call.id, output: content };
  }

  protected appendToolResults(results: CodexFunctionOutput[]): void {
