...
export interface AuthService {
  login(device?: boolean, notify?: (message:string)=>void, signal?: AbortSignal): Promise<AuthStatus>;
...
export class CodexAuthService implements AuthService {
  ...
  async login(device=false,notify=(m: string)=>console.log(m),signal?:AbortSignal):Promise<AuthStatus>{const token=device?await this.oauth.loginDevice(notify,signal):await this.oauth.loginBrowser(notify,120_000,signal);this.persist(token);return this.toStatus(this.store.getCredential(token.accountId)!);}
